...
import Halogen.Component (ComponentTree(..))

-- | Render a `HTML` document to a virtual DOM node
-- |
-- | The first argument is an event handler.
renderHTML :: forall p f g eff. (forall i. f i -> Aff (HalogenEffects eff) i) -> HTML (ComponentTree f g) (f Unit) -> V.VTree
renderHTML f = go
  where
  go :: HTML (ComponentTree f g) (f Unit) -> V.VTree
  go (Text s) = V.vtext s
  ...
  go (Slot (ComponentTree tree)) = go tree.html
